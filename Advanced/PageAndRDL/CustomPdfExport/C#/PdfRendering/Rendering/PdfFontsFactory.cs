	...
	internal sealed class PdfFontsFactory
	{
		...
		private class GdiFontResolver : PdfSharpFontResolver
		{
			...
			public byte[] GetFont(string faceName)
			{
				...
				var fontData = GetFontBytes(fontName, GetFontStyle(faceName), 0x66637474) ?? GetFontBytes(fontName, GetFontStyle(faceName), 0);
			...
			private static byte[] GetFontBytes(string fontName, System.Drawing.FontStyle fontStyle, int table, int size = 0)
			{
				...
				using (Graphics gTemp = Graphics.FromImage(_refBitmap))
				{
